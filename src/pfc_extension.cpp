    ...
    if (rc != 0) {
        ...
        if (exit_code == 127) {
            ...
            throw std::runtime_error(
                "PFC binary not found at '" + binary + "'.\n"
                "Install with:\n"
        ...
        if (exit_code == 1) {
            ...
            throw std::runtime_error(
                "PFC Community Mode daily limit reached (5 GB decompressed / day).\n"
                "Wait until midnight UTC, or get an unlimited license: "
                "https://github.com/ImpossibleForge/pfc-jsonl");
        }
        throw std::runtime_error(
            "PFC binary exited with code " + std::to_string(exit_code) +
...
static unique_ptr<FunctionData> PFCBind(
    ...
    if (bind_data->ts_from != 0 && bind_data->ts_to != 0 &&
        bind_data->ts_from > bind_data->ts_to) {
        throw BinderException(
            "read_pfc_jsonl: ts_from (" + std::to_string(bind_data->ts_from) +
