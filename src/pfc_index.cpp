...
PFCIndex PFCIndex::ReadFromFile(const std::string &bidx_path) {
    ...
    if (!f) {
        throw std::runtime_error(
            "Failed to read index header (file too small?): " + bidx_path);
    }

    if (std::memcmp(hdr.magic, "PFCI", 4) != 0) {
        throw std::runtime_error(
            "Invalid magic bytes in index file — not a PFC-JSONL v3.4+ index: " + bidx_path);
    }
    if (hdr.version != 1) {
        throw std::runtime_error(
            "Unsupported index version " + std::to_string(hdr.version) +
    ...
    const uint64_t expected_size =
        static_cast<uint64_t>(sizeof(PFCBidxHeader)) +
        static_cast<uint64_t>(hdr.block_count) * sizeof(PFCBidxRecord);

    if (file_size < expected_size) {
        throw std::runtime_error(
            "Index file is truncated — declared " + std::to_string(hdr.block_count) +
